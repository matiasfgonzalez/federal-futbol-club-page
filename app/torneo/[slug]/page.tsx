...
const getResultColor = (partido: IPartidoTorneo) => {
  ...
  const golesFederal = esFederalLocal
    ? partido.golesLocal
    : partido.golesVisitante;
  const golesRival = esFederalLocal
    ? partido.golesVisitante
    : partido.golesLocal;
  if (golesFederal > golesRival)
...
const calculateStandings = (
  ...
  const categoryMatches = partidos.filter(
    (p) => (p.categoria || 'A') === categoryId,
  );

  categoryMatches.forEach((p) => {
    if (standings[p.equipoLocal] && p.escudoLocal)
      standings[p.equipoLocal].teamLogo = p.escudoLocal;
    if (standings[p.equipoVisitante] && p.escudoVisitante)
      standings[p.equipoVisitante].teamLogo = p.escudoVisitante;
            ...
            {standings.map((team, index) => (
              ...
              >
                <td className="px-4 py-3 font-bold text-center">
                  <span
                    className={`flex items-center justify-center w-6 h-6 rounded-full mx-auto ${index < 4 ? 'bg-amber-400/20 text-amber-400' : 'bg-white/10 text-white/50'}`}
                  >
                    {index + 1}
                  </span>
                </td>
                <td className="px-4 py-3 font-semibold flex items-center gap-3">
                  ...
                  <span
                    className={
                      team.team.includes('Federal') ? 'text-amber-400' : ''
                    }
                  >
                    {team.team}
                  </span>
                ...
                <td className="px-2 py-3 text-center hidden sm:table-cell text-green-400/80">
                  {team.won}
                </td>
                <td className="px-2 py-3 text-center hidden sm:table-cell text-gray-400">
                  {team.drawn}
                </td>
                <td className="px-2 py-3 text-center hidden sm:table-cell text-red-400/80">
                  {team.lost}
                </td>
    ...
    >
      ...
      <div className="flex items-center justify-center gap-4">
        <div className="flex-1 text-right">
          <p
            className={`font-bold text-sm md:text-base mb-2 ${partido.equipoLocal.includes('Federal') ? 'text-amber-400' : 'text-white'}`}
          >
            {partido.equipoLocal}
          </p>
        ...
        <div className="flex items-center gap-2 bg-[#1b2f62] px-4 py-2 rounded-xl min-w-[80px] justify-center">
          {partido.estado === 'COMPLETADO' && partido.golesLocal !== null ? (
            ...
            <span className="text-xl md:text-2xl font-black text-white/50">
              VS
            </span>
          )}
        </div>
        ...
        <div className="flex-1 text-left">
          <p
            className={`font-bold text-sm md:text-base mb-2 ${partido.equipoVisitante.includes('Federal') ? 'text-amber-400' : 'text-white'}`}
          >
...
export default function TorneoPage() {
  ...
  const partidosCompletados = torneo.partidos.filter(
    (p) => p.estado === 'COMPLETADO',
  );
  const victorias = partidosCompletados.filter(
    (p) =>
      p.golesLocal !== null &&
      p.golesVisitante !== null &&
      p.golesLocal > p.golesVisitante,
  ).length;
  const empates = partidosCompletados.filter(
    (p) =>
      p.golesLocal !== null &&
      p.golesVisitante !== null &&
      p.golesLocal === p.golesVisitante,
  ).length;
  const derrotas = partidosCompletados.filter(
    (p) =>
      p.golesLocal !== null &&
      p.golesVisitante !== null &&
      p.golesLocal < p.golesVisitante,
  ).length;
  const golesFavor = partidosCompletados.reduce(
    (acc, p) => acc + (p.golesLocal || 0),
    0,
  );
  const golesContra = partidosCompletados.reduce(
    (acc, p) => acc + (p.golesVisitante || 0),
    0,
  );
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <span
                className={`text-xs font-semibold px-3 py-1.5 rounded-full border ${estadoTorneo.color}`}
              >
                {estadoTorneo.text}
              </span>
            </div>
          ...
          >
            ...
            {torneo.listaBuenaFe && (
              <Dialog>
                ...
                <DialogContent className="w-[95vw] max-w-4xl lg:max-w-6xl max-h-[90vh] p-0 overflow-hidden rounded-3xl border-0 bg-gradient-to-b from-[#1b2f62] to-[#152347] shadow-2xl">
                  ...
                  <div className="overflow-auto max-h-[calc(90vh-120px)] w-full bg-white/5 p-4 lg:flex lg:items-center lg:justify-center">
                    <div className="bg-white rounded-2xl overflow-hidden shadow-lg">
                      <img
                        src={torneo.listaBuenaFe}
                        alt={`Lista de Buena Fe - ${torneo.nombre}`}
                        className="w-full h-auto lg:w-auto lg:max-w-full lg:max-h-[calc(90vh-180px)] object-contain"
                      />
                    </div>
                  </div>
                </DialogContent>
          ...
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {[
                {
                  label: 'Partidos',
                  value: partidosCompletados.length,
                  icon: Calendar,
                },
      ...
      {(torneo.otrosPartidos || []).length > 0 && (
        <section className="py-12 md:py-16">
          <div className="container mx-auto px-4">
            ...
            {(torneo.categorias || [{ id: 'default', nombre: 'General' }]).map(
              (cat) => {
                  ...
                  <div key={cat.id} className="mb-12 last:mb-0">
                    ...
                    {Object.entries(fechasCat).map(([fecha, partidos]) => (
                      <div key={fecha} className="mb-6 last:mb-0">
                        <p className="text-sm font-semibold text-amber-400/70 text-center mb-3">
                          {fecha}
                        </p>
